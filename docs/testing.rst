...
1. ``maykinmedia-django``: backend tests, runs ``./bin/jenkins_django.sh``.
2. ``maykinmedia-js``: frontend tests, runs ``./bin/jenkins_js.sh``.

**develop** branch
------------------

1. ``maykinmedia-django-develop``: backend tests, runs ``./bin/jenkins_django.sh``.
2. ``maykinmedia-django-develop-js``: frontend tests, runs ``./bin/jenkins_js.sh``.

pull requests
-------------
1. ``maykinmedia-pr-django``: backend tests, runs ``./bin/jenkins_django_pr.sh``.
2. ``maykinmedia-pr-js``: frontend tests, runs ``./bin/jenkins_js.sh``.
3. ``maykinmedia-pr-sass``: checks that sass compiles, runs ``./bin/jenkins_sass.sh``.
4. ``maykinmedia-pr-isort``: checks that imports are correctly
