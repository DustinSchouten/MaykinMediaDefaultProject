...
2. Get the code:
   ...
   .. code-block:: bash
       ...
       git clone git@bitbucket.org:maykinmedia/maykinmedia.git
       cd maykinmedia
...
``src/maykinmedia/conf/local.py``. You can base this file on the
...
.. code-block:: bash
    ...
    python src/manage.py test maykinmedia
...
* ``DB_NAME``: name of the database for the project. Defaults to ``maykinmedia``.
* ``DB_USER``: username to connect to the database with. Defaults to ``maykinmedia``.
* ``DB_PASSWORD``: password to use to connect to the database. Defaults to ``maykinmedia``.
...
2. Start the database and web services:
   ...
   .. code-block:: bash
       ...
       docker-compose up -d
       Starting maykinmedia_db_1 ... done
       Starting maykinmedia_web_1 ... done
   ...
   .. code-block:: bash
       ...
       docker logs -f maykinmedia_web_1
...
3. Create an admin user and load initial data. If different container names
   ...
   .. code-block:: bash
       ...
       docker exec -it maykinmedia_web_1 /app/src/manage.py createsuperuser
       Username: admin
       ...
       Superuser created successfully.

       docker exec -it maykinmedia_web_1 /app/src/manage.py loaddata admin_index groups
...
.. _Github: https://github.com/maykinmedia/maykinmedia/
...
environment variables. See ``src/maykinmedia/conf/docker.py`` for
all settings.

.. code-block:: bash

    docker build -t maykinmedia
    docker run \
        -p 8000:8000 \
        -e DATABASE_USERNAME=... \
        -e DATABASE_PASSWORD=... \
    ...
    docker exec -it maykinmedia /app/src/manage.py createsuperuser
